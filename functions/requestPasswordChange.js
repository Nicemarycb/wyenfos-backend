...
//     const userId = userRecord.uid; // Get the actual Firebase UID of the user whose password will be changed
...
//     // Store the pending change request in Firestore with plain text password (to be hashed later)
//     await admin.firestore().collection("pendingChanges").doc(token).set({
//       userId,
//       newPassword, // Store plain text temporarily
//       type: "password",
...
//       html: `<p>A password change has been requested for the admin login account: <b>${emailToChange}</b>.</p><p>Click here to confirm and approve the password change: <a href="${approvalUrl}">${approvalUrl}</a></p><p>This link is valid for 24 hours.</p>`,
//     };
...
const requestPasswordChangeHandler = async (req, res) => {
  try {
    ...
    return res.status(200).json({message: "Confirmation email sent to default master admin email for approval. Check your inbox. Link expires in 3 hours."});
  } catch (error) {
    console.error("Error in requestPasswordChangeHandler:", error);
    return res.status(500).json({error: `Failed to request password change: ${error.message}`});
  }
